...
fn main() -> Result<()> {
    let seed: u64 = 200;
    let poisson_radius: f64 = 4.5;
    let margin: f64 = 0.08;
    let side: f64 = 1.5;

    let mut sketch = Sketch::new(&PageLayout::fabriano_24x32(Portrait), Uom::Mm, Debug::On);
    ...
    let circle = Circle::new(sketch.center(), 14.0);
    let center = sketch.center();

    for point in &distribution1 {
        let dist = sdf_circle(*point, &circle);
        let dist_abs = dist.abs() / 3.;
        sketch.group(0).add(Circle::new(
            point.centroid(),
            (1.0 - dist_abs.sin())
                * (point.distance(circle.center) / Vec2::new(0., 0.).distance(center))
                * side,
            // (1.0 - dist_abs.sin()) * (1.0 - point.distance(circle.center) / Vec2::new(0., 0.).distance(center)) * side,
        ));
    }
