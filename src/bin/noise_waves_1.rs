...
fn main() -> Result<()> {
    ...
    let col_size = sketch.as_rect().width / cols as f64;
    let row_size = sketch.as_rect().height / rows as f64;
    ...
    for y in 0..rows {
        ...
        let _ = &LineStr::new(point_set1).clip(&sketch.as_rect().scale_perc(0.97).to_linestr(true), false).iter().for_each(|l| {
            line_set1.add_lstr(&l);
        });
        let _ = &LineStr::new(point_set2).clip(&sketch.as_rect().scale_perc(0.97).to_linestr(true), false).iter().for_each(|l| {
            line_set2.add_lstr(&l);
        });
        let _ = &LineStr::new(point_set3).clip(&sketch.as_rect().scale_perc(0.97).to_linestr(true), false).iter().for_each(|l| {
            line_set3.add_lstr(&l);
        });
        let _ = &LineStr::new(point_set4).clip(&sketch.as_rect().scale_perc(0.97).to_linestr(true), false).iter().for_each(|l| {
    ...
    sketch.add_group(&line_set1, &Style::new("blue", "0.3mm"));
    sketch.add_group(&line_set2, &Style::new("red", "0.3mm"));
    sketch.add_group(&line_set3, &Style::new("black", "0.3mm"));
    sketch.add_group(&line_set4, &Style::new("gray", "0.3mm"));
