...
fn main() -> Result<()> {
    ...
    sketch.group(0).add(triangle[0].clone());
    sketch.group(0).add(triangle[1].clone());
    sketch.group(0).add(triangle[2].clone());
    for line in &hatch_fill(&triangle[0]) {
        sketch.group(1).add(line);
    }

    sketch.group(0).set_style(Style::new("black", "0.5mm"));
    sketch.group(1).set_style(Style::new("black", "0.5mm"));
